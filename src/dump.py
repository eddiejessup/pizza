...
class dump:
    ...
    def unscale_one(self, snap, x, y, z):
        if snap.xy == 0.0 and snap.xz == 0.0 and snap.yz == 0.0:
            ...
            atoms = snap.atoms
            if atoms != None:
                atoms[:, x] = snap.xlo + atoms[:, x] * h0 + \
                    atoms[:, y] * h5 + atoms[:, z] * h4
    ...
    def owrap(self, other):
        ...
        for snap in self.snaps:
            ...
            if self.lineflag == 2 or self.triflag == 2:
                self.objextra.owrap(snap.time, xprd, yprd,
                                    zprd, ids, atoms, iother, ix, iy, iz)
    ...
    def write(self, file, header=1, append=0):
        ...
        for snap in self.snaps:
            ...
            if header:
                ...
                if snap.boxstr:
                    print >>f, "ITEM: BOX BOUNDS", snap.boxstr
                else:
    ...
    def get_atom_data(self, isnap=None, timestep=None):
        if timestep is not None:
            isnap = np.where(self.time() > timestep)[0][0]
        ...
        for i in xrange(snap.natoms):
            for name, index in self.names.items():
                value = snap.atoms[i][index]
                datas[name].append(value)
        return datas
        ...
        if self.bondflag:
            ...
            for bond in bondlist:
                try:
                    ...
                    atom2 = atoms[j]
                    bonds.append(
                        [bond[0], bond[1], atom1[2], atom1[3], atom1[4],
            ...
            for i in xrange(data.nsnaps):
                if data.snaps[i].tselect:
                    print "%d atoms of %d selected in first step %d" % \
                          (data.snaps[i].nselect, data.snaps[i]
                           .natoms, data.snaps[i].time)
                    break
